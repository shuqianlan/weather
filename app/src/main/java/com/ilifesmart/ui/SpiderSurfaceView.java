...
public class SpiderSurfaceView extends CustomSurfaceView implements GestureDetector.OnGestureListener {
	...
	private int DEFAULT_POINT_COUNTS = 40;      // 随机点数量
	...
	private SpiderSurfaceView.Config mConfig;
	...
	private List<SpiderSurfaceView.CodePoint> mPointList = new ArrayList<>();

	public SpiderSurfaceView(Context context) {
		this(context, null);
	}

	public SpiderSurfaceView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public SpiderSurfaceView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void initialize(AttributeSet attrs) {
		mConfig = new SpiderSurfaceView.Config();
	...
	private void initCodePoints() {
		...
		for (int i = 0; i < DEFAULT_POINT_COUNTS; i++) {
			...
			mPointList.add(new SpiderSurfaceView.CodePoint(point_x, point_y, xa, ya));
		}
	}
	...
	public void doDraw(Canvas canvas, Paint paint) {
		canvas.drawColor(Color.WHITE); // 清屏操作. Fill the entire canvas' bitmap (restricted to the current clip) with the specified color,using srcover porterduff mode.
		if (mTouchY != -1 && mTouchX != -1) {
			canvas.drawPoint(mTouchX, mTouchY, mPointPaint);
		}

		for (SpiderSurfaceView.CodePoint currPoint:mPointList) {
			currPoint.x += currPoint.getXa();
			currPoint.y += currPoint.getYa();

			if (mTouchX != -1 && mTouchY != -1) {
				// 降速处理.
				drawLine(canvas, currPoint, new SpiderSurfaceView.CodePoint(mTouchX, mTouchY), mTouchPaint, 0.03);
			}
			...
			for (SpiderSurfaceView.CodePoint point:mPointList) {
				if (point != currPoint) {
	...
	private void drawLine(Canvas canvas, SpiderSurfaceView.CodePoint currPoint, SpiderSurfaceView.CodePoint point, Paint paint) {
		drawLine(canvas, currPoint, point, paint, 1.0);
	}

	private void drawLine(Canvas canvas, SpiderSurfaceView.CodePoint currPoint, SpiderSurfaceView.CodePoint point, Paint paint, double speedRatio) {
